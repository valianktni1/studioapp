...
async def create_tenant(body: TenantCreate, sa=Depends(get_current_super_admin)):
    ...
    tenant = {
        ...
        "website": None,
        "plan": body.plan,
        "storage_limit_bytes": PLANS[body.plan]["storage_limit_bytes"],
...
async def update_plan(tenant_id: str, body: PlanUpdate, sa=Depends(get_current_super_admin)):
    ...
    if body.plan:
        ...
        updates["storage_limit_bytes"] = PLANS[body.plan]["storage_limit_bytes"]
...
async def overview(sa=Depends(get_current_super_admin)):
    ...
    total_limit = sum(t.get("storage_limit_bytes", 0) for t in tenants)
